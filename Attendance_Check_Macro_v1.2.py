...
class Ui_mainwindow(object):
    ...
    def retranslateUi(self, mainwindow):                                 # 매크로 프로그램 화면
        ...
        self.label_4.setText(_translate("mainwindow", "v1.2"))
    ...
    def Checkbutton(self):
        ...
        while True : 
            ...
            if moving_type == 0 :       # 처음 실행했을 때
                for i in range(1, 3):       # 새글 피드 버튼 확인을 위한 준비
                    m.keyDown('alt')        # (매크로창이 맨 앞에 올라와 있으면 Alt+Tab 정상적으로 동작 X)
                    for j in range(1, 3):
                        m.press('tab')
                    m.keyUp('alt')
            ...
            while True :
                x, y = m.position()
                RGB = m.screenshot().getpixel((x, y))                        # 색깔 잡기
                if RGB == (52, 204, 108) or (255, 91, 114) or (255, 112, 61) or (147, 112, 240) :   # 출석체크 아이콘 버튼의 색깔을 찾으면
                    x, y = m.position()
